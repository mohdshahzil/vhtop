...
interface FoodItemsProps {
  title: string;
  foodItems: FoodItem[];
  totalCredits: number; // Add totalCredits as a prop
  setTotalCredits: React.Dispatch<React.SetStateAction<number>>; // Add setCredits as a prop
}

const FoodparkItems: React.FC<FoodItemsProps> = ({
  title,
  foodItems,
  totalCredits,
  setTotalCredits,
  ...
  const makePaymentandStore = async (totalPrice: number) => {
    try {
      ...
      // Subtract totalPrice from totalCredits and update it
      setTotalCredits((prevTotalCredits) => prevTotalCredits - totalPrice);
    } catch (error) {
      ...
      <Dialog>
        ...
        {selectedItem && (
          <DialogContent>
            ...
            <Button onClick={() => makePaymentandStore(totalPrice)}>
              Done
            </Button>
          </DialogContent>
        )}
